...
void sctpstr_cli(FILE *fp,int sock_fd,struct sockaddr *to,socklen_t tolen)
{
	...
	while (fgets(sendline,MAXLINE,fp) != NULL) {
		...
		// 使用sctp_opt_info來取得association的資訊
		length_status = sizeof(status);
		ret_value = sctp_opt_info(sock_fd,sri.sinfo_assoc_id,SCTP_STATUS,&status,&length_status);
		if (ret_value == -1) {
