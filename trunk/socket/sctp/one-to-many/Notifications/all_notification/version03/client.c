...
int main(int argc,char **argv)
{
	...
	int flags=1;
	ret_value = setsockopt(sock_fd,IPPROTO_SCTP,SCTP_NODELAY,&flags,sizeof(flags));
	if (ret_value == -1) {
		printf("setsockopt SCTP_NODELAY error\n");
		exit(-1);
	}
...
void sctpstr_cli(FILE *fp,int sock_fd,struct sockaddr *to,socklen_t tolen)
{
	...
	while (fgets(sendline,MAXLINE,fp) != NULL) {
		...
		ret_value = sctp_sendmsg(sock_fd,sendline,out_sz,to,tolen,0,0,sri.sinfo_stream,100,0);
		if (ret_value == -1) {
...
void sctpstr_cli_echoall(FILE *fp,int sock_fd,struct sockaddr *to,socklen_t tolen)
{
	...
	while(fgets(sendline,SCTP_MAXLINE-9,fp) != NULL) {
		...
		for (i=0;i<SERV_MAX_SCTP_STRM;i++) {
			...
			ret_value = sctp_sendmsg(sock_fd,sendline,sizeof(sendline),to,tolen,0,0,i,100,0);
			if (ret_value == -1) {
				printf("sctp_sendmsg error\n");
				exit(-1);
			}
