...
uint8_t *pdapi_recvmsg(int sock_fd,int *rdlen,SA *from,int *from_len,struct sctp_sndrcvinfo *sri,int *msg_flags)
{
	...
	at_in_buf = Sctp_recvmsg(sock_fd,sctp_pdapi_readbuf,sctp_pdapi_rdbuf_sz,from,from_len,sri,msg_flags);
	if (at_in_buf < 1) {
		*rdlen = at_in_buf;
		return (NULL);
	}
	while ((*msg_flags & MSG_EOR) == 0) {
		...
		rdsz = Sctp_recvmsg(sock_fd,&sctp_pdapi_readbuf[at_in_buf],left,NULL,&frmlen,NULL,msg_flags);
		at_in_buf += rdsz;
	}
	*rdlen = at_in_buf;
