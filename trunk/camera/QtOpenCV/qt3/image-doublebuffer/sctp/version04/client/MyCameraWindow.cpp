...
void MyCameraWindow::Recvdata()
{
	...
	else
	{
		...
		if (strcmp(buffer,IMAGE_OK)==0)
		{
			...
			if (ret <= 0)
				qWarning("send IMAGE_SPACE_PREPARE_OK error");
			else
			{
#ifdef REN_DEBUG
		...
		else if (strcmp(buffer,IMAGE_END)==0)
		{
			// 把圖片放到qlabel
			receiving = false;
			qWarning("total:%d",begin);
			begin = 0;
			qWarning("I got a image");
			setCameraImage();
		}
		else
		{
			...
			if (ret <= 0)
				qWarning("send IMAGE_RECVING error");
			else
			{
#ifdef REN_DEBUG
...
void MyCameraWindow::setCameraImage()
{
	...
	qWarning("height:%d\nwidth:%d",matrix.height,matrix.width);
...
void MyCameraWindow::timerEvent(QTimerEvent*)
{
	...
	{
		qWarning("still recving remain image");
	}
}
