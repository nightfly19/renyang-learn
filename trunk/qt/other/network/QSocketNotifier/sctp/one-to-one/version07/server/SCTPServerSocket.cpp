...
int SCTPServerSocket::recvMsg(int s)
{
	...
	qWarning("the peer address is %s",inet_ntoa(peer.sin_addr));
...
{
	int ret;
	ret = sctp_sendmsg(connfd,str.latin1(),str.length(),(struct sockaddr *) NULL,0,0,0,0,100,0);
	if (ret == -1) {
