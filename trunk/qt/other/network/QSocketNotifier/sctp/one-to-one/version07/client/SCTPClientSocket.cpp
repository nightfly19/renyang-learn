...
SCTPClientSocket::SCTPClientSocket(QObject *parent,const char *name):QObject(parent,name)
{
	...
	SctpSocketHandler::SctpSetMaxStream(connSock,5);
	SctpSocketHandler::SctpSetNoDelay(connSock);
	// 設定SCTP_NODELAY

	SctpSocketHandler::SctpSetRtoMax(connSock,10000);
	SctpSocketHandler::SctpSetRtoMin(connSock,1000);
	// 宣控所有的事件
	SctpSocketHandler::SctpTurnOnAllEvent(connSock);
...
{
	int ret;
	ret = sctp_sendmsg(connSock,str.latin1(),str.length(),(struct sockaddr *) NULL,0,0,0,0,100,0);
	if (ret == -1) {
