...
/*
 *  This code uses one of the Blowfish algorithm available
 *  at Bruce Schneider web site: http://www.schneier.com/
 *  Author: Unknown
 *
 *  The code has been adapted for IHU by Matteo Trotta
*/
...
Blowfish::Blowfish(char *pass, int len)
{
	passlen = MAX_PASSWD+1;
	if ((passwd = (char *) malloc(passlen*sizeof(char)))==NULL)
		throw Error(Error::IHU_ERR_MEMORY);
	memset(passwd, 0x0, passlen);
	setPasswd(pass, len);
}
	...
	if (passlen > 0)
  	{
		...
		{
			Work.byte.zero = passwd[(j++)%passlen];
			Work.byte.one = passwd[(j++)%passlen];
			Work.byte.two = passwd[(j++)%passlen];
			Work.byte.three = passwd[(j++)%passlen];
			PA[i] ^= Work.word;
		}
...
void Blowfish::setPasswd(char *pass, int len)
{
	reset();
	if (len > MAX_PASSWD)
		len = MAX_PASSWD;
	passlen = len;
	memcpy(passwd, pass, passlen);
	genSubKeys();
}
...
char* Blowfish::getPass()
{
	return passwd;
}

int Blowfish::getPassLen()
{
	return passlen;
}

bool Blowfish::isAsciiKey()
{
	for (int i=0; i<passlen; i++)
	{
		if (isascii(passwd[i]) == 0)
			return false;
	}
	return true;
}
