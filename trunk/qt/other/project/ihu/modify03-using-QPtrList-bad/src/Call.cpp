...
void Call::setPrimaddr(QString primaddr)
{
	...
	if (IPChanging == false)
	{
		...
		// renyang-modify - 表示剛剛改變完primary address, 短時間內不能再改primary address
		IPChanging = true;
...
void Call::resetStream()
{
#ifdef REN_DEBUG
	qWarning("Call::resetStream()");
#endif
	streamno = 1;
}
...
void Call::SlotAddressFail(QString address)
{
	...
	// renyang-modify - 若這一個fail的ip剛好是目前的prim address的話, 則要更新primary address
	if (address == callprimaddr)
	{
		// renyang-modify - 設定目前新的ip
		...
		{
			for (QStringList::Iterator it = availableIP.begin();it!=availableIP.end();++it)
			{
				if (*it != address)
				{
					setPrimaddr(*it);
		...
		transmitter->setStreamNo(transmitter->getStreamNo()+1);
	}
}
...
void Call::SlotAddressAvailable(QString address)
{
	...
	availableIP.append(address);
	qWarning("available account:%d",availableIP.count());
}
