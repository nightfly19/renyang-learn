...
void Call::SCTPnewConnection(int socketd,int protocol,struct sockaddr *addrs,int addrcnt)
{
#ifdef REN_DEBUG
	qWarning(QString("void Call::SCTPnewConnection(%1,%2,struct sockaddr *addrs)").arg(socketd).arg(protocol));
#endif
	try
	{
		sd = socketd;
		transmitter->SCTPnewConnection(sd,addrs,addrcnt);
	}
	catch (Error e)
	{
		abort();
...
void Call::re_notification_sctp()
{
#ifdef REN_DEBUG
	qWarning("Call::re_notification_sctp()");
#endif
